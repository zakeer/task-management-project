...
export default function Header() {
    ...
    const userToken = useSelector(state => {
        console.log(":: Header useSelector ::", state);
    ...
    return <header className="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full bg-blue-600 text-sm py-3 sm:py-0">
        <nav className="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
            <div className="flex items-center justify-between">
                <Link className="flex-none text-xl font-semibold text-white" to="/" aria-label="Brand">Brand</Link>
            ...
            <div id="navbar-collapse-with-animation" className="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
                <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
                    ...
                    {userToken ? <>
                        ...
                        <Link to="/add-task" className="font-medium text-white sm:py-6" >Add Task</Link>
                        <Link to="/categories" className="font-medium text-white sm:py-6" >Categories</Link>
                        <Link to="/add-category" className="font-medium text-white sm:py-6" >Add Category</Link>
